...
training_data = SpotifyDataset(sessions, 10000)
...
class DDQN(nn.Module):
    def __init__(self, encoder):
        ...
        self.dqn1 = DQN(encoder)
        self.dqn2 = DQN(encoder)
        self.fc = nn.Linear(4, 2)
        
    def forward(self, history, actions, next_song):
        x1 = self.dqn1(history, actions, next_song)
        x2 = self.dqn1(history, actions, next_song)
...
policy_dqn = DDQN(encoder1).to(device)
target_dqn = DDQN(encoder2).to(device)
